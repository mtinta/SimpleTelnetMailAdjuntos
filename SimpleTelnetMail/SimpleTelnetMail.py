...
class TelnetMail:
    ...
    def build_mime_message(self):
        "Construir mensaje MIME con texto y archivos adjuntos"
        mime_message = MIMEMultipart()
        mime_message["From"] = self.from_
        mime_message["To"] = ", ".join(self.to)
        mime_message["Subject"] = self.headers.get("Subject", "Sin asunto")
        ...
        for file_path in self.adjuntos:
            try:
    ...
    def send_mail(self):
        ...
        print("Construyendo el mensaje MIME...")
...
def main():
    ...
    for argument in optlist:
        ...
        elif argument[0] == "--adjuntos" or argument[0] == "-a":
            if argument[1]:
                adjuntos = argument[1].split(",")  # separa multiples archivos con comas
                arguments["adjuntos"] = adjuntos
